...
lr.test.result <-
  predict(lr, type = "raw", newdata = test.fixedColnames)
...
# Test data predictions
lr.test.result <-
  predict(tree, type = "raw", newdata = test.fixedColnames)
caret::confusionMatrix(data = lr.test.result, test.fixedColnames$condition)

# Simple plot: https://stat.ethz.ch/R-manual/R-devel/library/rpart/html/plot.rpart.html
png(
  paste0(imagesDirectory, "decision-tree-1.png"),
  width = 1200,
  height = 1200
)
...
png(
  paste0(imagesDirectory, "decision-tree-2.png"),
  width = 1200,
  height = 1200
)
rpart.plot(
...
# Test data predictions
lr.test.result <-
  predict(randomforest, type = "raw", newdata = test.fixedColnames)
caret::confusionMatrix(data = lr.test.result, test.fixedColnames$condition)
...
rownames(randomforest$finalModel$importance) <-
  colnames(train)[-ncol(train)]

varImpPlot(randomforest$finalModel, type = 2)
dev.print(
  png,
  paste0(imagesDirectory, "random-forest-variable-importance.png"),
  width = 600,
  height = 600
)
...
# Test data predictions
nn.test.result <-
  predict(nn, type = "raw", newdata = test.fixedColnames)
