...
png(
  paste0(imagesDirectory, "biomarker-fiedler-intensities.png"),
  width = 1200,
  height = 800
)

boxplot(
  binnedPeaksMatrix[, "1450.33683095267"] ~ binnedPeaksMatrix.conditions,
  ylab = "intensity",
  xlab = "condition",
  main = "Peak 1450.33683095267 by condition"
)
...
# Show the boxplot for a peak with p-val < 0.01
png(
  paste0(imagesDirectory, "biomarker-species-intensities.png"),
  width = 1200,
  height = 800
)

boxplot(
  binnedPeaksMatrix[, "1974.3176290975"] ~ binnedPeaksMatrix.conditions,
  ylab = "intensity",
  xlab = "species",
  main = "Peak 1974.3176290975 by species"
)
...
binnedPeaksMatrix <-
  asPresenceMatrix(binnedPeaksMatrix, "present", "absent")

# Test the first peak
contingencyTable <-
  table(factor(binnedPeaksMatrix[, 1], c("present", "absent")), binnedPeaksMatrix.conditions)
contingencyTable
...
for (i in 1:ncol(binnedPeaksMatrix)) {
  pvals[colnames(binnedPeaksMatrix)[i]] <-
    fisher.test(table(
      factor(binnedPeaksMatrix[, i], c("present", "absent")),
      binnedPeaksMatrix.conditions
    ))$p.value
}
pvals <- p.adjust(pvals, method = "fdr")
...
for (i in 1:ncol(binnedPeaksMatrix)) {
  pvals[colnames(binnedPeaksMatrix)[i]] <-
    fisher.test(table(
      factor(binnedPeaksMatrix[, i], c(1, 0)),
      binnedPeaksMatrix.conditions
    ))$p.value
}
pvals <- p.adjust(pvals, method = "fdr")
...
png(
  paste0(imagesDirectory, "biomarker-cancer-top20-heatmap.png"),
  width = 1200,
  height = 800
)

topPeaksMatrixWithPvals <-
  t(binnedPeaksMatrix[, names(pvals[pvals < 0.05])])
rownames(topPeaksMatrixWithPvals) <-
  paste(rownames(topPeaksMatrixWithPvals),
        " (pval: ",
        as.character(format(pvals[pvals < 0.05])),
        ")")
...
avgPeaks <-
  meanConditionSpectra(
    unique(binnedPeaksMatrix.conditions),
    binnedPeaksMatrix,
    binnedPeaksMatrix.conditions
  )
...
names.both.filter <-
  intersect(names.pValue.filter, names.logFC.filter)
...
png(
  paste0(imagesDirectory, "biomarker-cancer-volcano-plot.png"),
  width = 900,
  height = 600
)
...
points(logFC[names.logFC.filter], -log10(pvals[names.logFC.filter]), col =
         colors[2], pch = 20)
points(logFC[names.pValue.filter], -log10(pvals[names.pValue.filter]), col =
         colors[3], pch = 20)
points(logFC[names.both.filter], -log10(pvals[names.both.filter]), col =
         colors[4], pch = 20)

#
# Label the significant peaks
#
textxy(logFC[names.both.filter],-log10(pvals[names.both.filter]),
