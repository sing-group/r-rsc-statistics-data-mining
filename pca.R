...
sample_conditions <- sapply(rownames(binnedPeaksMatrix), function(x) substr(x,1,1))

png(paste0(imagesDirectory, "pca-replicates.png"), width=640, height=480)
pca2d(pca, group=sample_conditions, components=c(1,2), legend="bottomright")
dev.off()

# Uncomment the following line to show the 3D visualiation of the PCA
# pca3d(pca, group=sample_conditions, components=c(1,2,3), legend="bottomright")
...
sample_conditions <- sapply(rownames(consensusBinnedPeaksMatrix), function(x) substr(x,1,1))
...
pca2d(pca, group=sample_conditions, components=c(1,2), legend="bottomright", biplot=TRUE)
dev.off()
